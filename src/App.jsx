      ...
      {!isFound ? (
        <div className='pt-16 px-16 flex justify-center'>
          <div className='w-2/5 p-5 shadow-md bg-white bg-opacity-40 rounded-xl text-gray-200 max-lg:w-full'>
            <h1 className='text-3xl max-sm:text-xl text-center'>City <span className='text-white text-5xl max-sm:text-4xl'>"{errorMessage}"</span> is not found. </h1>
            <h1 className='text-3xl max-sm:text-xl text-center'> Please try again!</h1>
          </div>
        </div>
          ...
          <div className="flex justify-center">
              ...
              <div className='w-1/3 max-lg:w-full'>
                <input type="search" id="default-search" className="w-full p-4 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 outline-none" placeholder="Search City..." required
                ...
                <div className='relative z-50'>
                  ...
                  : provinceData.filter(item => {
                    ...
                    return (search && fullName.startsWith(search) && search !== fullName)
                  }).slice(0, 5).map((item,index)=>(
                    <div className='p-1 cursor-pointer hover:bg-white last:hover:rounded-xl last:hover:rounded-t-none' 
                    key={index}
                    onClick={()=>{setCountry(item.name_en)}}>
                    {item.name_en}
                    </div>
                  ))}
                  </div>
              ...
              <button type="button" className="h-[53.6px] p-4 ml-2 text-sm font-medium text-white bg-blue-700 rounded-lg border border-blue-700 hover:bg-blue-800 focus:ring-2 focus:outline-none focus:ring-blue-300 outline-none"
              ...
              <div className='container-left relative flex flex-row flex-wrap gap-5 items-center bg-white bg-opacity-5 shadow-md rounded-xl justify-center py-5 pr-5 bg-cover bg-center w-5/12 max-xl:w-full'
